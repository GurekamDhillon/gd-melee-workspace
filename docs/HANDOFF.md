...
**Landed so far (phase 3a/3b/3c, commits `f079ccdf3`/`60c27b64e`/`8e2a9f63d`):** the interpreter
RUNS in-game — `onLoad` (once at fighter creation), `onFrame` (per frame), `onActionStateChange`
(ftcolanim.c ×4), `onReapplyAttr` (ftchangeparam.c). The 8 specials are registered at their
`ftCo_Attack100`/`ftCo_SpecialS`/`ftCo_SpecialAir` sites but are input-gated and firing one hits the
**float-marshalling gap** (the bridge marshals `r3-r10` only, not `f1-f8`).

**Still to do:** float/sret marshalling across the bridge; `MoveLogic` (slot 3) is a *data table*
(move-ID → anim/IASA/phys/coll callbacks), not code — needs the m-ex move-table runtime;
`onDoubleJump`/`onUSmash`/`OnItemPickup` need bespoke sites; the guest RENDER callback
(`GObj_SetupGXLink` → `ftDrawCommon_80080E18`) re-enters fine but hangs the render loop without
m-ex's render-context tables, so it stays deferred.

...
namespace GDMelee
{
    static class L
    {
        ...
        static readonly Dictionary<string, string> Spanish = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            { "Keyboard only (ignore controllers and the adapter)", "Solo teclado (ignora los mandos y el adaptador)" },
            { "Close this launcher when the game starts", "Cerrar este launcher al iniciar el juego" },
            { "Game volume: {0}%", "Volumen del juego: {0}%" },
            { "Keyboard plays as:", "El teclado juega como:" },
            { "Port 1, shared with a controller (default)", "Puerto 1, compartido con un mando (predeterminado)" },
            { "Port 2 (a separate player)", "Puerto 2 (otro jugador)" },
            { "Port 3", "Puerto 3" },
            { "Port 4", "Puerto 4" },
            { "PLAY", "JUGAR" },

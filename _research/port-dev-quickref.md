...
- DO NOT take or capture screenshots. Visual verification is delegated to the human user.
...
- **Shim boundary.** Every SDK symbol the game imports is `gw_<name>`; a shim defines `gw_X`, game code calls `X`, and the linker resolves it. `extern void gw_wait_idle(void);` from game source under `TARGET_PC` is fine.
